...
namespace :gh_pages do
  ...
  task push: "asset_pages:precompile" do
    ...
    dest_dir.join(".nojekyll").open("wb")
    ...
    dest_dir.join("CNAME").open("wb") { |f| f.write("#{project_cname}\n") } \
      if !project_cname.nil?
    ...
    if repo_path_pattern.match(main_repo_origin.url).nil?
    ...
    Rugged::Remote.add(repo, "origin", main_repo_origin.url) \
      if Rugged::Remote.lookup(repo, "origin").nil?
